...
class Redi
{
	...
	{
		return $this->curl(REDI_RESTAURANT_API.RESERVATION.$this->ApiKey.'/'.$categoryID, POST, json_encode($params));
	}
	...
	{
		return $this->curl(REDI_RESTAURANT_API.CATEGORY.$this->ApiKey.'/'.$placeID, POST, json_encode($params));
	}
	...
	public function setServiceTime($categoryID, $timeSet)
	{
		return $this->curl(REDI_RESTAURANT_API.CATEGORY.$this->ApiKey.'/'.$categoryID.'/time',
			PUT,
			json_encode(array ('timeSet' => $timeSet)));
	}
	...
	{
		return $this->curl(REDI_RESTAURANT_API.SERVICE.$this->ApiKey.'/'.$categoryID, POST, json_encode($params));
	}
	...
	{
		return $this->curl(REDI_RESTAURANT_API.USER, POST, json_encode($params));
	}

	public function setPlace($placeID, $params)
	{
		return $this->curl(REDI_RESTAURANT_API.PLACE.$this->ApiKey.'/'.$placeID, PUT, json_encode($params));
	}

	public function createPlace($params)
	{
		return $this->curl(REDI_RESTAURANT_API.PLACE.$this->ApiKey, POST, json_encode($params));
	}
