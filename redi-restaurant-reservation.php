...
if ( ! class_exists( 'ReDiRestaurantReservation' ) ) {
	...
	class ReDiRestaurantReservation {
		...
		private function step1( $placeID, $post ) {
		...
		function redi_restaurant_ajax() {
			...
			switch ( $_POST['get'] ) {
				case 'step1':
					echo json_encode( $this->step1( $placeID, $_POST ) );
					break;
				...
				case 'cancel':
					...
					if ( isset( $this->options['EmailFrom'] ) && $this->options['EmailFrom'] == EmailFrom::Disabled || isset( $this->options['EmailFrom'] ) && $this->options['EmailFrom'] == EmailFrom::WordPress ) {
						$params['DontNotifyClient'] = 'true';
					}
